...
**아주 가끔은 아침 알람이 아예 안 울립니다.** GitHub이 바쁘면 예약을 그냥 건너뛰는데,
건너뛰었다고 알려주지도 않습니다(GitHub 공식 문서에 적혀 있는 동작입니다).
그래서 **예비 알람을 9시 25분과 11시 25분에 하나씩 더 걸어뒀습니다.** 첫 알람이 사라져도
다음 것이 대신 돌고, 이미 브리핑이 나갔으면 예비 알람은 아무것도 하지 않고 바로 끝냅니다
(중복 전송도, 중복 요금도 없습니다).

**정리하면 — 브리핑은 오전 중에 옵니다.** 점심때까지 안 왔다면 그날은 실패한 것이니
저장소의 `Actions` 탭에서 빨간 표시를 확인하세요.
